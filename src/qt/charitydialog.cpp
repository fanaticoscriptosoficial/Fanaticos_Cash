    ...
    model(0)
{
    ...
    ui->charityAddressEdit->setPlaceholderText(tr("Enter FanaticosCash Address"));
...
//QString charitiesFCHImage[100];
//QString charitiesFCHURL[100];
...
void StakeForCharityDialog::setModel(WalletModel *model)
{
    ...
    if (strAddress.IsValid() && nPer > 0 )
    {
        ...
        if (strAddress.ToString().c_str() == QString(FOUNDATION) || strAddress.ToString().c_str() == QString(FOUNDATION_TEST) )
        {
            ...
            ui->message->setText(tr("Thank you for giving to The FanaticosCash Foundation \n\n"));
        ...
        else
        {
            ...
            if (i==100) // STS FCH must be a 'manual' entered address
            {
                ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900; }");
        ...
        if (pwalletMain->IsLocked())
        {
            ...
            ui->message->setText(ui->message->text() + "<br />" + "You must unlock your FanaticosCash software <br />for Staking for Charity to proceed to: " + strAddress.ToString().c_str());
        }
    }
}
...
{   
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        // clear current combo box entires, leaving the first 2, "manual" and "FCH foundation"
        for (i=2; i < n; i++ ) {
        ...
        foreach (const QJsonValue &value, json_array) {
            ...
            if (fTestNet) qDebug() << json_obj["FCHaddress"].toString();
            charitiesAddress[i] = json_obj["FCHaddress"].toString();
            ...
            //charitiesFCHImage[i] = json_obj["FCHimg"].toString();
...
void StakeForCharityDialog::on_enableButton_clicked()
{
    ...
    {
        ui->message->setStyleSheet("QLabel { color: red; font-weight: 900;}");
        ui->message->setText("Please unlock your FanaticosCash software for sending (not staking only) <br /> for FanaticosCash Staking for Charity to proceed.");
        return;
    }
    else if (fWalletUnlockStakingOnly)
    {
        ui->message->setStyleSheet("QLabel { color: red; font-weight: 900;}");
        ui->message->setText("Please unlock your FanaticosCash software for spending <br /> (not staking only) for FanaticosCash Staking for Charity to proceed.");
        return;
    }
    ...
    ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900;}");
    ui->message->setText("FanaticosCash Staking For Charity enabled to:<br /> " + QString(address.ToString().c_str()) + " at a rate of " + QString::number(nCharityPercent) + "%");
...
void StakeForCharityDialog::on_disableButton_clicked()
{
    ...
    ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900;}");
    ui->message->setText(tr("FanaticosCash Dynamic Stake for Charity is now off and saved off."));
    return;
}
...
{
    QPixmap IMGpixmap;
    QPixmap FCHIMGpixmap;
    FCHIMGpixmap.load(":/images/Wallet_Logo");
    if (index==0)
    {
        ui->charityAddressEdit->clear();
        ...
        ui->message->setText("Please enter the FanaticosCash address <br />or select from the drop-down <br />and then click 'Enable'");
        ui->charityAddressEdit->setFocus();
        ui->label_IMG->clear();
        ui->label_HREF->clear();
    }
    ...
    else if (index > 1)
    {
        ...
        ui->charityAddressEdit->setStyleSheet("border-color: #35473c; color:35473c;");
        ui->label_5->setStyleSheet("QLabel {color: #018457;}");
        ui->message->setText(charitiesAsk[index-1].replace("<a href","<a style='color: #1ab06c;' href") + "<br />Click the 'Enable' button above to save <br />and start FanaticosCash Dynamic Stake for Charity");
