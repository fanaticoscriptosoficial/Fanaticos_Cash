...
void Shutdown(void* parg)
{
    ...
    RenameThread("FanaticosCash-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("FanaticosCash version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  FanaticosCashd [options]                     " + "\n" +
                  "  FanaticosCashd [options] <command> [params]  " + _("Send command to -server or FanaticosCashd") + "\n" +
                  "  FanaticosCashd [options] help                " + _("List commands") + "\n" +
                  "  FanaticosCashd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "fanaticoscash:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("FanaticosCash"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("FanaticosCash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: FanaticosCash.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: FanaticosCashd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 6384 or testnet: 6382)") + "\n" + //Portas P2P Main e Testnet
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 6385 or testnet: 6383)") + "\n" + //Portas RPC Main e Testnet - Francis
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  FanaticosCash is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("FanaticosCash version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "FanaticosCash server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("FanaticosCash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // FanaticosCash: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // FanaticosCash: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of FanaticosCash") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart FanaticosCash to complete") << "\n";
