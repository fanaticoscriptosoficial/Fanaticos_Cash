...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_INVALIDKEYODE          : return "OP_INVALIDKEYODE";
    default:
        return "OP_UNKNOWN";
    }
}
        ...
        case OP_NOP6: case OP_NOP7: case OP_NOP8: case OP_NOP9: case OP_NOP10:
                ...
                case OP_CHECKLOCKTIMEVERIFY:
                {
                    ...
                    if (!(flags & SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY)) {
                        // treat as a NOP2 is not enabled
                        break;
                    }
                    if (stack.size() < 1)
                        return false;
                ...
                case OP_CHECKSEQUENCEVERIFY:
                {
                    if (!(flags & SCRIPT_VERIFY_CHECKSEQUENCEVERIFY)) {
                        // treat as a NOP3 if not enabled
                        break;
                    }
                    if (stack.size() < 1)
                        return false;
                    ...
                    if ((nSequence.getuint64() & SEQUENCE_LOCKTIME_DISABLE_FLAG) != 0)
                        break;
...
public:
    bool
    Get(uint256 hash, const std::vector<unsigned char>& vchSig, const CPubKey& pubKey)
    {
        LOCK(cs_sigcache);
...
unsigned int CScript::GetSigOpCount(bool fAccurate) const
{
    ...
    opcodetype lastOpcode = OP_INVALIDKEYODE;
    while (pc < end())
    {
        opcodetype opcode;
        if (!GetOp(pc, opcode))
