...
unsigned int nTargetSpacing = 3 * 60; // 180 seconds
unsigned int nRetarget = 30;
unsigned int nStakeMinAge = 2 * 60 * 60; // 2 hours min - Tempo Mínimo Stake - Francis
unsigned int nStakeMaxAge = 12 * 60 * 60; // 12 hours max - Tem máximo Stake - Francis
...
int nCoinbaseMaturity = 5; //Maturação Mineração - Francis
...
const string strMessageMagic = "FanaticosCash Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // FanaticosCash: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // FanaticosCash: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if(pindexBest->nHeight > 1000) //1ª FASE
    {
        nSubsidy = 4.5 * COIN;
        return nSubsidy + nFees;
    }

    else if(pindexBest->nHeight > 350400) //2ª FASE
    {
        nSubsidy = 4 * COIN;
        return nSubsidy + nFees;
    }

    else if(pindexBest->nHeight > 1226400) //3ª FASE
    {
        nSubsidy = 3.5 * COIN;
        return nSubsidy + nFees;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    else if(pindexBest->nHeight >= 175200){
        nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE_FASE02;
    }
    else if(pindexBest->nHeight >= 350400){
    ...
    else if(pindexBest->nHeight >= 700800){
        nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE_FASE05;
    }
    else if(pindexBest->nHeight >= 876000){
...
// FanaticosCash: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes FanaticosCash fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // FanaticosCash: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // FanaticosCash: clean up wallet after disconnecting coinstake
    BOOST_FOREACH(CTransaction& tx, vtx)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // FanaticosCash: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // FanaticosCash: track money supply and mint amount info
...
// FanaticosCash: total coin age spent in transaction, in the unit of coin-days.
...
// FanaticosCash: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // FanaticosCash: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // FanaticosCash: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // FanaticosCash: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // FanaticosCash: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig, int height) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // Fanaticoscash: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // FanaticosCash: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // FanaticosCash: check proof-of-stake
    ...
    // FanaticosCash: ask for pending sync-checkpoint if any
...
printf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), pblock->hashPrevBlock.ToString().substr(0,20).c_str());
  if (pfrom) {
      ...
      // Ask this guy to fill in what we're missing
          ...
          // FanaticosCash: getblocks may not obtain the ancestor block rejected
    ...
    // FanaticosCash: if responsible for sync-checkpoint send it
...
// fanaticoscash: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FanaticosCash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // FanaticosCash: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // FanaticosCash: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // FanaticosCash: tell downloading node about the latest block if it's
