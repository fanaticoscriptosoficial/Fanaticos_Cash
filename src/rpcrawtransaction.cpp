...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid FanaticosCash address: ")+input.get_str());
...
Value signrawtransaction(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 1 && params[1].type() != null_type)
    {
        ...
        BOOST_FOREACH(Value& p, prevTxs)
        {
            ...
            if ((unsigned int)nOut >= mapPrevOut[outpoint].size())
                mapPrevOut[outpoint].resize(nOut+1);
